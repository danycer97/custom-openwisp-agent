...
-- custom module for bond interface info
print("---------------------------")
...
local function stampaTabella(tabella, prefisso)
  ...
  for chiave, valore in pairs(tabella) do
      if type(valore) == "table" then
          print(prefisso .. chiave .. ":")
          stampaTabella(valore, prefisso .. "  ")
      else
          print(prefisso .. chiave .. ":", valore)
      end
  end
end
for _, interface in ipairs(custom_interfaces) do
  -- stampaTabella(interface)
  if interface.name == 'bond0' and interface.family=='inet' then
    print("stats bond0")
...
print("custom table :")
-- print(bond_interfaces.up)

stampaTabella(bond_interfaces)

print("---------------------------")

-- custom module plc stats
print("---------------------------")

local plc_stats = {}
local file_plc_stats = io.open('/root/plc_stats.txt','r')
if file_plc_stats then
  plc_stats = file_plc_stats:read("*a")
  file_plc_stats:close()
end
print(plc_stats)
plc_stats = monitoring.utils.split(plc_stats, ',')
stampaTabella(plc_stats)
local plc_tab = {}
plc_tab.name = 'plc1'
plc_tab.mac = plc_stats[1]
plc_tab.rssi = plc_stats[2]
plc_tab.beacon_sequence_ID = plc_stats[3]
plc_tab.AGC = plc_stats[4]
plc_tab.Authentication = plc_stats[5]

stampaTabella(plc_tab)
...
table.insert(host_interfaces, bond_interfaces)
table.insert(host_interfaces, plc_tab)  -- aggiunge tabella custom bond alla tabella contenente tutte le interfacce
